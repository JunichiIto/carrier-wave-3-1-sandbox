...
class PostTest < ActiveSupport::TestCase
  ...
  test "image.present? は S3 アクセスを発生させなくなった (#2776 修正)" do
    # master(f635d88)では blank? / present? は「ファイルが割り当てられているか」
  ...
  test "ファイルが実在しなくても presence バリデーションは valid になる(セマンティクスの変化)" do
    ...
    # 3.1.x では blank? がストレージを見るためファイル不在なら invalid(HEAD 5 回)だったが、
    # master では識別子カラムがあれば valid となり、S3 アクセスも発生しない
    assert post.valid?
    assert_empty @requests
  end
