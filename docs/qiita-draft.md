...
「version の実体がない」状態は珍しいエッジケースではなく、通常の運用で自然に発生します。

- **version を後からアップローダーに追加した** — 既存レコードには version の実体がないため、`recreate_versions!` を全レコードに流すまでの間(あるいは流し忘れたレコードで恒常的に)この状態になります
- **version の条件(`if:`)や名前を後から変更した** — 以前の定義では生成されなかったレコードが、新しい定義では「あるはず」と判定されるようになります
- **`store!` や `recreate_versions!` の部分失敗** — CarrierWave の保存は親と各 version を順に PUT するだけでトランザクション性がないため、途中で失敗すると親だけ保存された状態になり得ます

たとえば「preview があれば表示、なければ元画像にフォールバック」という表示分岐を `preview.present?` で書いていた場合、3.1 系では実体を確認するので正しくフォールバックしますが、4.0 では present? が true を返すため**壊れた URL をそのまま表示してしまいます**。このような判定は `exists?` への書き換えが必須です。
