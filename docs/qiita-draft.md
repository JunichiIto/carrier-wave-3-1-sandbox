...
3.0 系の `blank?` はストレージに問い合わせないため、上記の HEAD はいずれも発生しません。サンプルアプリの Gemfile を `gem "carrierwave", "3.0.7"` に差し替えて `bundle update carrierwave` すると、HEAD 発生をアサートしているテストが軒並み失敗する(= S3 アクセスが発生しない)ことで確認できます。
